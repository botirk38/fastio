...
- Do not use `tokio::fs`. The backend owns an `Arc<std::fs::File>` and runs all I/O via `tokio::task::spawn_blocking`.
- Keep all I/O off Tokio runtime worker threads via `spawn_blocking` with owned or `Arc`-cloned data.
